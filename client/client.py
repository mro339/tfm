...
# Cada cliente usa una partición distinta. Es decir, si hay 2 clientes, el cliente 1 usa la primera mitad del conjunto de entrenamiento y el cliente 2 usa la segunda mitad.
...
num_clients = 2 #Número total de clientes (debe coincidir con el número de instancias de cliente que se ejecutan)
...
x_train_c = x_train[start:end] #Datos de entrenamiento para este cliente
y_train_c = y_train[start:end] #Etiquetas de entrenamiento para este cliente

print(f"📡 Cliente {client_id} arrancado con {len(x_train_c)} imágenes")
...
def build_model(): #Construir un modelo de red neuronal convolucional simple para clasificar las imágenes de MNIST
    model = tf.keras.Sequential([ #Modelo secuencial apilando capas linealmente. signifca que la salida de una capa es la entrada de la siguiente
        tf.keras.layers.Conv2D(32, 3, activation="relu", input_shape=(28, 28, 1)), #Capa convolucional con 32 filtros de tamaño 3x3 y función de activación ReLU
        ...
        tf.keras.layers.Dense(64, activation="relu"), #Capa densa (fully connected) con 64 neuronas y función de activación ReLU
